...
void print_usage(void)
{
    ...
    fprintf(stderr,
            ...
            "                       RND -- Random\n"
            "\n"
            " --optqr           : reuse the Q from the QR facto to calculate the cond est in the 1st iteration of QDWH\n"
            "\n"
            " --optid           : take advantage of the Id structure during the QDWH iteration \n"
            "\n"
            " --cond            : generate a matrix using the provided cond number \n"
            "\n"
...
#define GETOPT_STRING "bc:o:g::p:P:q:Q:N:M:K:A:B:C:i:t:T:s:S:xXv::hd:r:y:V:a:R:m:YZW:"
...
static struct option long_options[] =
{
    ...
    /* QDWH options */
    {"optqr",       no_argument,        0, 'Y'},
    {"Y",           no_argument,        0, 'Y'},
    {"optid",       no_argument,        0, 'Z'},
    {"Z",           no_argument,        0, 'Z'},
    {"cond",        required_argument,  0, 'W'},
...
static void parse_arguments(int *_argc, char*** _argv, int* iparam, double *cond)
{
    extern char **environ;
    int opt = 0;
    int rc, c;
    ...
    do {
        ...
        switch(c)
        {
            ...
            case 'z': iparam[IPARAM_HNB] = iparam[IPARAM_HMB] = atoi(optarg); break;
                ...
                /* QDWH parameters */
            case 'Y': iparam[IPARAM_OPTQR]  = 1; break;
            case 'Z': iparam[IPARAM_OPTID]  = 1; break;
            case 'W': *cond = atof(optarg); break;
...
static void print_arguments(int* iparam, double *cond)
{
    int verbose = iparam[IPARAM_RANK] ? 0 : iparam[IPARAM_VERBOSE];
    ...
    if(verbose > 1) fprintf(stderr, "#+++++ nodes x cores + gpu  : %d x %d + %d (%d+%d)\n"
                            ...
                            iparam[IPARAM_OPTQR], iparam[IPARAM_OPTID], *cond);
...
static void iparam_default(int* iparam)
{
    ...
    iparam[IPARAM_OPTQR]  = 0;
    iparam[IPARAM_OPTID]  = 0;
}
...
dague_context_t* setup_dague(int argc, char **argv, int *iparam, double *cond)
{
#ifdef DAGUE_PROF_TRACE
    argvzero = argv[0];
...
#endif
    parse_arguments(&argc, &argv, iparam, cond);
