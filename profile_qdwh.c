          ...
          dague_context_t *dague )
{
    ...
    *flops = 0.0;
    ...
    //if( optcond )
    {
        ...
        */
        ...
        fprintf(stderr,
                "\ndtrtri: Time  \t\tGflop/s \n"
                "\t%e  \t%8.2f\n\n",
                sync_time_elapsed, FLOPS_DTRTRI( N )/1e9/sync_time_elapsed);
        }
    ...
    while(conv > tol3 || it < itconv ) {
        ...
        if ( rank == 0 ) {
        fprintf(stderr,
                "\ndgeqrf_full: Time  \tGflop/s \n"
                "\t%e  \t%8.2f\n\n",
                sync_time_elapsed, (flops_dgeqrf+flops_dorgqr)/1e9/sync_time_elapsed);
        }
            ...
            *itQR = *itQR +1;
            ...
            if (sym){
                ...
                fprintf(stderr,
                        "\ndgemm: Time  \t\tGflop/s \n"
                        "\t%e  \t%8.2f\n\n",
                        sync_time_elapsed, FLOPS_DGEMM( M, N, N )/1e9/sync_time_elapsed);
                }
            ...
            fprintf(stderr,
                    "\ndposv: Time  \t\tGflop/s \n"
                    "\t%e  \t%8.2f\n\n",
                    sync_time_elapsed, (FLOPS_DPOTRF( M ) + 2.*FLOPS_DTRSM( PlasmaLeft, M, N ))/1e9/sync_time_elapsed);
             }
