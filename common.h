...
enum iparam_t {
  ...
  IPARAM_OPTQR,        /* Use QR for cond estimation and reuse the Q in the first iteration */
  IPARAM_OPTID,        /* Take advantage of the Id structure */
  IPARAM_SIZEOF
};
...
#define PASTE_CODE_IPARAM_LOCALS(iparam)                                \
    ...
    int optqr = iparam[IPARAM_OPTQR];                                   \
    int optid = iparam[IPARAM_OPTID];                                   \
    ...
    (void)scheduler;(void)optqr;(void)optid;(void)butterfly_level; \
    (void)check_inv;(void)random_seed;(void)matrix_init;
...
#define PASTE_CODE_PROGRESS_KERNEL(DAGUE, KERNEL)                       \
    ...
    PROFILING_SAVE_iINFO("PARAM_OPTQR", iparam[IPARAM_OPTQR]);  \
    PROFILING_SAVE_iINFO("PARAM_OPTID", iparam[IPARAM_OPTID]);  \
    if(loud >= 5 && rank == 0) {                                        \
