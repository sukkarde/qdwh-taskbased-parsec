 ...
 *
 *  dplasma_dgeqrf_id_setrecursive - Set the recursive size parameter to enable
 *  recursive DAGs.
 *
 *******************************************************************************
 *
 ...
 ******************************************************************************/
void
dplasma_dgeqrf_id_setrecursive( dague_handle_t *handle, int hnb )
{
    dague_dgeqrf_id_handle_t *dague_dgeqrf_id = (dague_dgeqrf_id_handle_t *)handle;

    if (hnb > 0) {
        dague_dgeqrf_id->smallnb = hnb;
    }
}
 ...
 * @ingroup dplasma_complex64
 *
 * dplasma_dgeqrf_id_New - Generates the handle that computes the QR factorization
 ...
 *   - dplasma_dgeqrf_id_param_New() parameterized with trees for tall and skinny
 *     matrices
 *   - dplasma_dgeqrf_id_param_New() parameterized with systolic tree if
 ...
 * are for now local. You can follow the code of dplasma_dgeqrf_id_rec() as an
 * example to do this.
 *
 ...
 *          destroy with dplasma_dgeqrf_id_Destruct();
 *
 *******************************************************************************
 *
 * @sa dplasma_dgeqrf_id
 * @sa dplasma_dgeqrf_id_Destruct
...
{
    dague_dgeqrf_id_handle_t* handle;
    int ib = T2->mb;

    handle = dague_dgeqrf_id_new( (dague_ddesc_t*)A1,
                                  (dague_ddesc_t*)A2,
                                  (dague_ddesc_t*)T2,
                                  ib, optid,
                                  NULL, NULL );
    ...
    dague_private_memory_init( handle->p_tau, T2->nb * sizeof(double) );

    handle->p_work = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t));
    dague_private_memory_init( handle->p_work, ib * T2->nb * sizeof(double) );

    /* Default type */
    dague_matrix_add2arena( handle->arenas[DAGUE_dgeqrf_id_DEFAULT_ARENA],
                            dague_datatype_double_t,
                            matrix_UpperLower, 1, A1->mb, A1->nb, A1->mb,
                            DAGUE_ARENA_ALIGNMENT_SSE, -1 );

    /* Lower triangular part of tile without diagonal */
    /* dague_matrix_add2arena( handle->arenas[DAGUE_dgeqrf_id_LOWER_TILE_ARENA], */
    /*                         dague_datatype_double_t, */
    /*                         matrix_Lower, 0, A1->mb, A1->nb, A1->mb, */
    /*                         DAGUE_ARENA_ALIGNMENT_SSE, -1 ); */

    /* Upper triangular part of tile with diagonal */
    dague_matrix_add2arena( handle->arenas[DAGUE_dgeqrf_id_UPPER_TILE_ARENA],
                            dague_datatype_double_t,
                            matrix_Upper, 1, A1->mb, A1->nb, A1->mb,
                            DAGUE_ARENA_ALIGNMENT_SSE, -1 );

    /* Little T */
    dague_matrix_add2arena( handle->arenas[DAGUE_dgeqrf_id_LITTLE_T_ARENA],
                            dague_datatype_double_t,
 ...
 * @ingroup dplasma_complex64
 *
 *  dplasma_dgeqrf_id_Destruct - Free the data structure associated to an handle
 *  created with dplasma_dgeqrf_id_New().
 *
 *******************************************************************************
 *
 * @param[in,out] handle
...
dplasma_dgeqrf_id_Destruct( dague_handle_t *handle )
{
    ...
    dague_matrix_del2arena( dague_dgeqrf_id->arenas[DAGUE_dgeqrf_id_DEFAULT_ARENA   ] );
    //dague_matrix_del2arena( dague_dgeqrf_id->arenas[DAGUE_dgeqrf_id_LOWER_TILE_ARENA] );
    dague_matrix_del2arena( dague_dgeqrf_id->arenas[DAGUE_dgeqrf_id_UPPER_TILE_ARENA] );
    dague_matrix_del2arena( dague_dgeqrf_id->arenas[DAGUE_dgeqrf_id_LITTLE_T_ARENA  ] );

    dague_private_memory_fini( dague_dgeqrf_id->p_work );
    dague_private_memory_fini( dague_dgeqrf_id->p_tau  );
    free( dague_dgeqrf_id->p_work );
    free( dague_dgeqrf_id->p_tau  );
 ...
 * @ingroup dplasma_complex64
 *
 * dplasma_dgeqrf_id - Computes the QR factorization a M-by-N matrix A:
 * A = Q * R.
 *
 * The method used in this algorithm is a tile QR algorithm with a flat
 * reduction tree. It is recommended to use the super tiling parameter (SMB) to
 ...
 * This variant is good for square large problems.
 * For other problems, see:
 *   - dplasma_dgeqrf_id_param() parameterized with trees for tall and skinny
 *     matrices
 *   - dplasma_dgeqrf_id_param() parameterized with systolic tree if computation
                   ...
                   tiled_matrix_desc_t *T2 )
{
    ...
    if ( (A2->mt != T2->mt) || (A2->nt != T2->nt) ) {
        dplasma_error("dplasma_dgeqrf_id", "T2 doesn't have the same number of tiles as A2");
        return -101;
    }

    dague_dgeqrf = dplasma_dgeqrf_id_New(optid, A1, A2, T2);

    if ( dague_dgeqrf != NULL ) {
        dague_enqueue(dague, (dague_handle_t*)dague_dgeqrf);
        dague_context_wait(dague);
        dplasma_dgeqrf_id_Destruct( dague_dgeqrf );
    }
 ...
 * @ingroup dplasma_complex64
 *
 * dplasma_dgeqrf_id_rec - Computes the QR factorization a M-by-N matrix A:
 ...
 ******************************************************************************/
int
dplasma_dgeqrf_id_rec( dague_context_t *dague,
                       int optid,
                       tiled_matrix_desc_t *A1,
                       tiled_matrix_desc_t *A2,
                       tiled_matrix_desc_t *T2, int hnb )
{
    ...
    if ( dague_dgeqrf != NULL ) {
        dague_enqueue( dague, (dague_handle_t*)dague_dgeqrf );
        dplasma_dgeqrf_id_setrecursive( (dague_handle_t*)dague_dgeqrf, hnb );
        dague_context_wait( dague );
        dplasma_dgeqrf_id_Destruct( dague_dgeqrf );
